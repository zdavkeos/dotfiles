...
;        '( "help " (0 font-lock-keyword-face t))
        '( "\\(if\\|IF\\) " (0 font-lock-builtin-face t))
...
;        '( "xcopy " (0 font-lock-keyword-face t))
		'( " [0-9]+\\.?[0-9]+ " (0 font-lock-preprocessor-face t))
;       '( " == " (0 font-lock-operator-face t))
        ;; These are special overrides for the above
        '( "^:[a-zA-Z]+[ \t]*$" (0 font-lock-reference-face t))
		; mark 'goto' targets are refences, but the 'goto' itself as a key word
        '( "^[ \t]*\\(goto\\|GOTO\\) [^ \t\n]+[ \t]*$" (0 font-lock-reference-face t))
        '( "^[ \t]*\\(goto\\|GOTO\\) " (0 font-lock-builtin-face t))
		; make the echo'ed string highlighed, but the 'echo' as a keyword
        '( "^[ \t]*@?\\(echo\\|ECHO\\) .*$" (0 font-lock-string-face t))
		'( "^[ \t]*@?\\(echo\\|ECHO\\)" (0 font-lock-keyword-face t))
		; make set special
		'( "^[ \t]*@?\\(set\\|SET\\)[ \t]+[a-zA-Z_]+" (0 font-lock-warning-face t))
		'( "^[ \t]*@?\\(set\\|SET\\) " (0 font-lock-builtin-face t))
        '( "%[^ %]+%" (0 font-lock-variable-name-face t))
        '( "%\\(~[a-zA-Z]+\\|\\)[0-9*]" (0 font-lock-variable-name-face t))
        '( "%%[a-zA-Z]" (0 font-lock-variable-name-face t))
        ; comments are the very last to cover-up anything, but keep 'rem' as a key word
        '( "^[ \t]*\\(rem\\|REM\\) [^\n]*$" (0 font-lock-comment-face t))
		'( "^[ \t]*\\(rem\\|REM\\) " (0 font-lock-comment-delimiter-face t))
        )
